...
run_crumblr_per_cohort <- function(seu, cohort_name) {
  ...
  # Save to CSV
  out_path <- paste0("Files/crumbler_results_age_", cohort_name, ".csv")
...
# Define cohorts and known sample sizes
cohorts <- list(
  Bat = 23,
  OFC = 69,
  McLean = 47,
...
library(dplyr)
# List your CSV files
files <- list.files(path = "Files", pattern = "^crumbler_results_age_.*\\.csv$", full.names = TRUE)

all_data <- do.call(rbind, lapply(files, function(f) {
  df <- read.csv(f)
  df$cohort <- sub("crumbler_results_age_(.*)\\.csv", "\\1", basename(f))
  df$SE <- df$logFC / df$t
  df
}))
...
# === 7. Save plot ===
ggsave("Figures/Meta_cohort_heatmap_age.png", plot = p, width = 28, height = 3, dpi = 300)
...
# === 7. Save plot ===
ggsave("Figures/Meta_cohort_heatmap_age_sst.png", plot = p, width = 10, height = 3, dpi = 300)
