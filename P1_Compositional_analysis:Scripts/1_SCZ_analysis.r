...
# === Remove oligodendrocytes from psychad === #
...
# HBCC
oligo_cells <- WhichCells(HBCC, expression = subclass.x == "Oligo" & Age < 70)
non_oligo_cells <- WhichCells(HBCC, expression = subclass.x != "Oligo" & Age < 70)

# randomly sample 20% of oligos
keep_oligos <- sample(oligo_cells, size = round(0.2 * length(oligo_cells)))

# subset object
HBCC <- subset(HBCC, cells = c(non_oligo_cells, keep_oligos))


# MSSM
oligo_cells <- WhichCells(MSSM, expression = subclass.x == "Oligo" & Age < 70)
non_oligo_cells <- WhichCells(MSSM, expression = subclass.x != "Oligo" & Age < 70)

keep_oligos <- sample(oligo_cells, size = round(0.2 * length(oligo_cells)))

MSSM <- subset(MSSM, cells = c(non_oligo_cells, keep_oligos))


#HBCC <- subset(HBCC, subset = subclass.x != "Oligo" & Age < 70)
#MSSM <- subset(MSSM, subset = subclass.x != "Oligo" & Age < 70)
...
filter_seurat_by_age_and_cells <- function(seu, min_cells = 500) {
  seu <- subset(seu, subset = Age < 70)
...
MSSM <- MSSM %>% filter_seurat_by_age_and_cells()
HBCC <- HBCC %>% filter_seurat_by_age_and_cells()
...
run_crumblr_per_cohort <- function(seu, cohort_name) {
  ...
  # Save to CSV
  out_path <- paste0("Files/crumbler_results_", cohort_name, ".csv")
...
# List your CSV files
files <- c(
  "Files/crumbler_results_Bat.csv",
  "Files/crumbler_results_HBCC.csv",
  "Files/crumbler_results_McLean.csv",
  "Files/crumbler_results_MSSSM.csv",
  "Files/crumbler_results_MtSinai.csv",
  "Files/crumbler_results_OFC.csv"
)
...
# === 7. Save plot ===
ggsave("Figures/SCZ_meta_cohort_heatmap2.png", plot = p, width = 28, height = 3, dpi = 300)
...
# Create a named vector of colors for CellTypes based on meta-analysis significance
celltype_colors <- setNames(
  dplyr::case_when(
    meta_plot_data$sig_label == "+" ~ "red",
    meta_plot_data$sig_label == "/" ~ "red",
    TRUE ~ "black"
  ),
  meta_plot_data$CellType
)

sst_order <- c(
  "Sst_1", "Sst_4", "Sst_5", "Sst_7",
  "Sst_3", "Sst_19", "Sst_9", "Sst_10", "Sst_13", "Sst_12", "Sst_11", "Sst_20", "Sst_22", "Sst_23", "Sst_25", "Sst_2"
)
...
plot_data_sub_sst <- plot_data %>%
  filter(
    grepl("^Sst_", CellType)
  )
  plot_data_sub_sst$CellType <- factor(plot_data_sub_sst$CellType, levels = rev(sort(unique(plot_data_sub_sst$CellType))))
  plot_data_sub_sst$CellType <- factor(plot_data_sub_sst$CellType, levels = sst_order)



p <- ggplot(plot_data_sub_sst, aes(x = CellType, y = Cohort, fill = estimate)) +
  geom_tile(color = "white", linewidth = 0.3) +  # white grid lines
  geom_text(aes(label = sig_label), size = 4) +  # significance "+" on top
  scale_fill_gradient2(
    low = "blue", mid = "white", high = "red",
  ...
  theme(
    axis.text.x = element_text(
      angle = 45,
      hjust = 1,
      color = celltype_colors[levels(plot_data_sub_sst$CellType)]  # ← the only edit
    ),
    axis.ticks = element_blank(),
    panel.grid = element_blank()
  ) +
...
# === 7. Save plot ===
ggsave("SCZ_meta_cohort_heatmap_sub_sst.png", plot = p, width = 10, height = 3, dpi = 300)
